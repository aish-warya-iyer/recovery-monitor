...
def baseline_for(patient_id: str, before: str) -> list[dict]:
    ...
    for r in db.all_("SELECT s.result_json, v.rep_labels_json FROM sessions s JOIN reviews v ON v.session_id = s.id "
                     "WHERE s.patient_id = ? AND v.decision = 'approve' AND s.created_at < ? "
                     "AND s.exercise = 'squat' ORDER BY s.created_at DESC", patient_id, before):
    ...
    try:
        ...
        if s["exercise"] == "squat":
            ...
            result = analyze_video(str(video), "squat", protocol, annotated_path=str(folder / "annotated.mp4"),
                                   ...
                                   baseline=baseline_for(s["patient_id"], s["created_at"]))
