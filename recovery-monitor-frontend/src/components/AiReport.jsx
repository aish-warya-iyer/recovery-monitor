...
export default function AiReport({ session, onUpdated }) {
  ...
  return (
    <Panel title="AI draft report" action={action}
      subtitle={`Draft for you to check · ${r.source === 'llm' ? `${r.model}, on this device` : 'template (model unavailable)'}${r.checks?.numbers_verified ? ' · numbers checked against the measurements' : ''}`}>
      {r.red_flags?.length > 0 && (
        <Note tone="error">
          <strong><ShieldAlert size={13} /> Red flags (fixed safety rules)</strong>
      ...
      <div className="report">
        ...
        <section>
          <h4>Do they match? <span className={`badge ${cls}`}>{label}</span></h4>
          <p>{r.agreement_note}</p>
        </section>
        ...
        <section className="next-step"><h4><Sparkles size={13} /> Suggested next step (draft)</h4><p>{r.suggested_next_step}</p></section>
