...
export default function PhysioQueue() {
  ...
  const assignedPatientIds = new Set(
    (intakes.data ?? [])
      .filter((intake) => intake.assigned_therapist_id)
      .map((intake) => intake.patient_user_id),
  );
  ...
  return (
    <div className="page care-dashboard">
      ...
      <Panel title="Patient requests" subtitle="Choose which patients you want to take on.">
        {intakes.data?.length ? <div className="intake-request-list">{intakes.data.map((intake) => {
          const assigned = Boolean(intake.assigned_therapist_id);
          return <div className="intake-request" key={intake.id}>
            <div><strong>{intake.patient?.name || intake.patient?.email || intake.patient_user_id}</strong><span>{intake.affected_areas?.join(', ')} · {assigned ? 'Assigned to you' : intake.status}</span><small>{intake.notes || 'New patient care request'}</small></div>
            <div className="intake-actions">
              {assigned ? <span className="intake-assigned-label">Care relationship active</span> : <>
                <button className="primary-button" onClick={async () => { try { await api.claimIntake(intake.id); intakes.reload(); patients.reload(); } catch (e) { setDecisionError(e); } }}>Accept patient</button>
                <button className="secondary-button" onClick={async () => { try { await api.declineIntake(intake.id); intakes.reload(); } catch (e) { setDecisionError(e); } }}>Decline</button>
              </>}
            </div>
          </div>;
        })}</div> : <div className="empty-state intake-empty"><Inbox size={22} /><p>No patient requests yet.</p><small>When a patient submits an issue intake, it will appear here for you to accept or decline.</small></div>}
      </Panel>

      <Panel title="Patients" subtitle="Open a patient to review their trend and adjust their plan.">
        <div className="patient-grid">
          {patients.data?.map((p) => {
            const assigned = assignedPatientIds.has(p.id);
            return <button key={p.id} className={`patient-card ${assigned ? '' : 'unassigned'}`} disabled={!assigned} onClick={() => go(`/physio/patient/${p.id}`)}>
              ...
              <div>
                ...
                <span>{assigned ? `Last session ${fmtDateTime(p.last_session_at)} · ${p.sessions_awaiting_review} awaiting review` : 'Awaiting care-team assignment'}</span>
              </div>
            </button>;
          })}
        </div>
